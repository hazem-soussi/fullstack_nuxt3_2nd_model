...
import Tiptap from "~~/components/elements/Tiptap.vue";
 ...
 const { data: questions, pending, refresh, error } = await useFetch<IQuestion[]>(() => `/api/ask-hazem/search?search=${searchInput.value}`, {server:false})
     ...
     <div class="w-full md:w-1/3">
         ...
         <Transition name="fade" v-if="!pending" v-for="question in questions">
             <NuxtLink v-if="question"
                 ...
                 :to="`/ask-hazem/question/${question.id}`">
